...
public class NegotiationException {
    ...
    // 구매제안 생성 검증 메소드
    public void validateCreateException(NegotiationCreateDto dto) {
    ...
    public void validateUpdateWriterOrPassword(NegotiationEntity entity, NegotiationUpdateDto dto) {
    ...
    // 구매제안의 작성자 혹은 비밀번호 검증 메소드
    public void validateDeleteWriterOrPassword(NegotiationEntity entity, NegotiationDeleteDto dto) {
        if (!entity.getWriter().equals(dto.getWriter()) || !entity.getPassword().equals(dto.getPassword())) {
            throw new IllegalArgumentException(NOT_SAME_WRITER_PASSWORD_MESSAGE);
        }
    }
